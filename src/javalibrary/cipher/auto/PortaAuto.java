...
public class PortaAuto implements IForceDecrypt {
	...
	public String tryDecode(String cipherText, EncryptionData data, ILanguage language, Output output, ProgressValue progressBar, JTextField mostLikely) {		
		...
		for(int length = minKeywordLength; length <= maxKeywordLength; length++)
			progressBar.addMaxValue(THIRTEEN.pow(length));
		
		String startText = "";
		PortaTask pt = new PortaTask(cipherText, language, output, progressBar);
		
		for(int length = minKeywordLength; length <= maxKeywordLength; length++)
			this.run(pt, length - startText.length(), 0, startText);
		
		return pt.plainText;
	}
	
	public static class PortaTask implements KeyCreation {
		...
		public PortaTask(String cipherText, ILanguage language, Output output, ProgressValue value) {
