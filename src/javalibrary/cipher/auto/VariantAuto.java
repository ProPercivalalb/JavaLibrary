...
public class VariantAuto implements IForceDecrypt {
	...
	public String tryDecode(String cipherText, EncryptionData data, ILanguage language, Output output, ProgressValue progressBar) {		
		...
		VariantTask vt = new VariantTask(cipherText, language, output, progressBar);
		
		for(int length = minKeywordLength; length <= maxKeywordLength; length++)
			this.run(vt, length, 0, "");
		
		return vt.plainText;
	}
	
	public static class VariantTask implements KeyCreation {
		...
		public VariantTask(String cipherText, ILanguage language, Output output, ProgressValue value) {
